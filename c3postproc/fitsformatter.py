...
def format_fits(chain, extname, types, units, nside, burnin, polar, component, fwhm, nu_ref, procver, filename, bndctr, restfreq, bndwid,):
    print()
    print(f"Formatting and outputting {filename}")
    header = get_header(extname, types, units, nside, polar, component, fwhm, nu_ref, procver, filename, bndctr, restfreq, bndwid)
...
def get_data(chain, extname, component, burnin, fwhm, nside, types):
    ...
    if extname.endswith('FREQMAP'):
        ...
        # stddev data
        amp_stddev = h5handler(input=chain, dataset=f"tod/{component}/map", min=burnin, max=None, output="map",fwhm=fwhm, nside= nside,command= np.std)
...
def get_header(extname, types, units, nside, polar, component, fwhm, nu_ref, procver, filename, bndctr, restfreq, bndwid):
