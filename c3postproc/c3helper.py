...
def c3help(op, nsysargs):
    ...
    if str(op) == "stddev":
        if nsysargs < 7:
            print("Usage: ")
            print(
                "c3pp stddev [.h5 filename] [h5 dataset (ex. cmb/sigma_l)] [sample range min] [sample range max] (OPTIONAL: [-smooth FWHM]) [output name (name.fits/name.dat)]"
            )
            print("example: ")
            print("c3pp stddev chain_c0001.h5 cmb/amp_map 10 20 -smooth 60 cmb_amp_stddev_60arcmin.fits")
            print(
        ...
        if nsysargs < 7:
            print("Usage: ")
            print(
                "c3pp mean [.h5 filename] [h5 dataset (ex. cmb/sigma_l)] [sample range min] [sample range max] (OPTIONAL: [-smooth FWHM]) [output name (name.fits/name.dat)]"
            )
            print("example: ")
            print("c3pp mean chain_c0001.h5 cmb/amp_map 10 20 -smooth 60 cmb_amp_mean_60arcmin.fits")
        ...
        if nsysargs < 3:
            ...
            print("-title                   Set title (Upper right), has LaTeX functionality. Ex. $A_{s}$. DOES NOT SUPPORT SPACES CURRENTLY")
            print("-unit                    Set unit (Under color bar), has LaTeX functionality. Ex. $\mu$ DOES NOT SUPPORT SPACES CURRENTLY")
            print()
        ...
        if nsysargs < 6:
            print("Usage:   ")
            print(
                "c3pp sigma_l2fits [.h5 file root (root_XXXXX.h5)] [h5 dataset (optional, default cmb/sigma_l)] [number of chains] [burn-in (sample at which finishes burn-in)] [out file (name.fits)]"
            )
            print(
                "This tool converts the .h5 dataset from Commander3 into .fits dateset suitable for Commander1 BR and GBR estimator analysis (See comm_like_tools for further information about BR and GBR post processing)."
            )
            quit()
        ...
        if nsysargs < 3:
            ...
            quit()
    elif str(op) == "dlbin2dat":
        if nsysargs < 6:
            print("Usage:   ")
            print("c3pp dlbin2dat [.h5 filename] [sample min] [sample max] [binfile]")
            print("Example: ")
            print("c3pp dlbin2dat chain_c0001.h5 1 20 bins_lmax2000_multires_v1.dat ")
            print(
...
def operations():
    operations_list = ["stddev", "mean", "plot", "sigma_l2fits", "h5map2fits", "alm2fits" "dlbin2dat"]
